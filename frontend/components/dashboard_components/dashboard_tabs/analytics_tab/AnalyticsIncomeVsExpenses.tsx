            ...
            const fetchData = async () => {
                ...
                const data = await Promise.all(
                    months.map(async (month) => {
                        const deposits = await getTransactionDetails(userId, ['deposit_amount'], [], false, month)
                        const income = deposits
                            .reduce((sum, deposit) => sum + (Number(deposit.deposit_amount) || 0), 0);

                        const transactions = await getTransactionDetails(userId, ['withdrawal_amount'], [], false, month)
