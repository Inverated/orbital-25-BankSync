...
import IncomeExpensesExpand from "./IncomeExpensesExpand";
...
export default function Overview() {
    ...
    const [lastMonthIncome, setLastMonthIncome] = useState(0.0);
    const [lastMonthExpenses, setLastMonthExpenses] = useState(0.0);
    ...
    useEffect(() => {
        ...
        setLastMonthIncome(transArray
            .filter(t => t.transaction_date >= lastMonthStart && t.transaction_date <= lastMonthEnd)
        ...
        setLastMonthExpenses(transArray
            .filter(t => t.transaction_date >= lastMonthStart && t.transaction_date <= lastMonthEnd)
            .reduce((sum, t) => sum + (t.withdrawal_amount || 0), 0)
        )
    ...
    const [showExpandedIncome, setShowExpandedIncome] = useState(false);
    const [incomeDate, setIncomeDate] = useState<Dayjs | null>(null);

    const [showExpandedExpenses, setShowExpandedExpenses] = useState(false);
                ...
                <div id="expanded_account" className="hidden">
                    {accountArray.map((account, index) =>
                        <div key={account.id} className="rounded-lg">
                            ...
                            {index < accountArray.length - 1 && <hr className="my-2 border-gray-300" />}
                        </div>
                    )}
                </div>
            </div>

            <div className="flex flex-col space-y-5 space-x-5 sm:flex-row justify-between w-3/4">
                <div className="justify-items-start w-1/1 h-full py-5 px-7 border border-gray-300 border-2 hover:cursor-pointer rounded-lg">
                    <button 
                        onClick={() => {
                            if (showExpandedIncome) {
                                setShowExpandedIncome(false);
                                setIncomeDate(null);
                            } else {
                                setShowExpandedIncome(true);
                            }
                        }}
                        className="flex flex-row w-full items-center justify-between hover:cursor-pointer"
                    >
                        ...
                        <div className="bg-gray-100 rounded-lg py-2 px-3">
                            <div className="text-med text-blue-400">
                                (${lastMonthIncome.toFixed(2)} earned in the past month)
                            </div>
                        </div>
                    </button>

                    {showExpandedIncome && (
                        <div className="flex flex-col items-center justify-center py-4 px-2 space-y-2 w-full">
                            <IncomeExpensesExpand category={"income"} />
                    ...
                    <button 
                        onClick={() => {
                            if (showExpandedExpenses) {
                                setShowExpandedExpenses(false);
                                setExpensesDate(null);
                            } else {
                                setShowExpandedExpenses(true);
                            }
                        }}
                        className="flex flex-row w-full items-center justify-between hover:cursor-pointer"
                    >
                        ...
                        <div className="bg-gray-100 rounded-lg py-2 px-3">
                            <div className="text-med text-red-400">
                                (${lastMonthExpenses.toFixed(2)} spent in the past month)
                            </div>
                        </div>
                    </button>

                    {showExpandedExpenses && (
                        <div className="flex flex-col items-center justify-center py-4 px-2 space-y-2 w-full">
                            <IncomeExpensesExpand category={"expenses"} />
